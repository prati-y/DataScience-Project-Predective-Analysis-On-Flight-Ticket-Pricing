...
data = df.select('searchDate','flightDate',"startingAirport", "destinationAirport", "fareBasisCode","isBasicEconomy", "isRefundable", "seatsRemaining","totalTravelDistance","segmentsDepartureTimeEpochSeconds", "segmentsArrivalTimeEpochSeconds" ,"segmentsAirlineCode", "segmentsEquipmentDescription", "segmentsDurationInSeconds" , "segmentsCabinCode", "baseFare")
data = data.withColumn("isBasicEconomy", col("isBasicEconomy").cast("int"))
data = data.withColumn("isRefundable", col("isRefundable").cast("int"))

#date to mon,day,year for search and flight
split_col = split(col('searchDate'), '/')
data = data.withColumn('SearchMonth', split_col.getItem(0).cast('int'))
data = data.withColumn('SearchDay', split_col.getItem(1).cast('int'))
data = data.withColumn('SearchYear', split_col.getItem(2).cast('int'))

split_col = split(col('flightDate'), '/')
data = data.withColumn('flightMonth', split_col.getItem(0).cast('int'))
data = data.withColumn('flightDay', split_col.getItem(1).cast('int'))
data = data.withColumn('flightYear', split_col.getItem(2).cast('int'))
count = data.count()

# Print the count
print("The dataset has", count, "records.")
data.printSchema()
data.show()
...
catVect = VectorAssembler(inputCols = ["isBasicEconomy", "startingAirportIdx", "destinationAirportIdx", "segmentsAirlineCodeIdx","segmentsCabinCodeIdx","segmentsEquipmentDescriptionIdx", "flightMonth","flightDay","flightYear", "SearchMonth", "SearchDay","SearchYear", "isRefundable"], outputCol="catFeatures")
...
gbt = GBTRegressor(labelCol="baseFare", featuresCol="features", maxBins=64)
